...
\section{LOV state}
\begin{itemize}
	\item{\st{evol nb Vocabs}  => mongo}
	\item{\st{cumul evol Vocabs creation date/last modif date | first last} => mongo}
	\item{\st{vocabs expressivity} => mongo}

	\item{\st{Median nb version per vocab | range} => ES}
	\item{\st{nb elems (classes / properties / instances) | range} => ES}
	\item{\st{Median classes / properties / instances} => ES}
	\item{\st{nb agents (organisations / persons)} => ES}
	
	\item{\st{Perc vocabs per language} => mongo}
	\item{\st{Distribution nb lang /vocabs} => mongo}
	\item{Perc. vocabs with agents => mongo}

	\item{\st{dist nb vocab / voaf rel type | nb | median} => mongo}
\end{itemize}

As of today, the LOV database contains over 47,000 RDF vocabulary terms, with 28,800 properties and 19,000 classes. The evolution of the number of vocabularies in the LOV is illustrated in figure \ref{fig:evolLOV}.
...
\begin{tabular}{lrr}
\hline
\textbf{Type} & \textbf{Count} &  \textbf{Med per vocab} \\ \hline
Vocabularies & 469 & \\ 
Classes & 18,336 & 9 \\ 
Properties & 27,152 & 16 \\ 
Instances & 4,672 & 0 \\ 
Datatypes & 94 & 0 \\ 
...
voaf:metadataVoc & 2396 \\ 
voaf:specializes & 1155 \\ 
voaf:extends & 946 \\ 
owl:imports & 329 \\ 
voaf:hasEquivalencesWith & 190 \\ 
voaf:generalizes & 49 \\ 
voaf:hasDisjunctionsWith & 15 \\ 
\hline  
\end{tabular}
\label{tab:voaf}
\end{table}
...
\begin{figure}[htb]
   ...
   \caption{\label{fig:evolLOV} Evolution of the number of vocabularies in LOV from March 2011 to May 2015.}
\end{figure}

\begin{figure}[htb]
\input{PGFPlots/LOVCreaEvol.tex}
\caption{\label{fig:evolLOV} Distribution of the vocabularies by creation date.}
\end{figure}

\begin{figure}[htb]
\input{PGFPlots/LOVModifEvol.tex}
\caption{\label{fig:evolLOV} Distribution of the vocabularies by last modification date.}
\end{figure}

\begin{figure}[htb]
\input{PGFPlots/LOVLangDist.tex}
\caption{\label{fig:evolLOV} Distribution of the vocabularies per language count.}
\end{figure}


\section{System Components and Features}
	Figure \ref{fig:arch} shows an overview of the LOV components architecture. It is composed of four main components. Each components provides a set of features detailed in the following subsections.
