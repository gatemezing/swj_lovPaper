...
\TODO{link Table 3 with the text}
 \begin{table}[h!tb]
\caption{Type of elements searched from January to June 2015 by users in LOV logs with a keyword.}
\begin{tabular}{lrr}
\hline
\textbf{Element Type searched} & \textbf{Nb searches} & \textbf{\% searches} \\
\textbf{with keyword(s)} & & \\ \hline
Term & 80,728 & 92.84\% \\
Vocabulary & 5,918 & 6.81\% \\
Agent & 306 & 0.35\% \\
\hline  
\end{tabular}
...
\TODO{link Table 4 with the text}
 \begin{table}[h!tb]
\caption{Type of elements searched from January to June 2015 by users in LOV logs.}
\begin{tabular}{lrr}
\hline
...
\begin{tabular}{ |l|l| }
  ...
  $t$ & A vocabulary term (class, property, instance \\
       & or datatype): $t \in V \& t \in URI$ \\
  \hline
\end{tabular}
...
\begin{equation}
\begin{split}
Score(t,Q) =tf(t,V) * idf(t,\mathbb{V}) * boost(t) \\
: \forall t\left\{\exists q_i \in Q: t \in \sigma(q_i)\right\}
\end{split}
\end{equation}
...
\todo{add how to use/integrate the results of the new LOD cloud statistics in LOV vocabularies, or continue the work of harmonizing the 649 vocabs from new LOD Cloud stats} 
