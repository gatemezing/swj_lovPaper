...
There are many ways two vocabularies can be interlinked. Let's consider two vocabularies $V1$ and $V2$ such as $V1$ contains a class $c1$ and a property $p1$ and $V2$ contains a class $c2$ and a property $p2$. Relationships between these two vocabularies can be of the following types (the lines numbers in brackets correspond to real examples presented in listing \ref{list:voaf}):
		\begin{description}
			...
			\item [Import.] some terms from $V2$ are reused in complement with $V1$ to capture the semantic of the data (lines 1 to 2).
...
dul:TimeInterval
		owl:disjointWith 
		ontopic:SubjectSpace.
\end{lstlisting}
=======
			\item [Specialization.] $V1$ defines some subclasses or subproperties (or local restrictions) of $V2$ (lines 3 to 6).

			\item [Generalization.] $V1$ defines some superclasses or superproperties of $V2$ (lines 7 to 9).

			\item [Extension.] $V1$ extends the expressivity of $V2$ (lines 10 to 13).

			\item [Equivalence.] $V1$ declares some equivalent classes or properties with $V2$ (lines 14 to 18).

			\item [Disjunction.] $V1$ declares some disjunct classes with $V2$ (lines 19 to 21).
