...
Over the last four years, the Linked Open Vocabularies initiative has gathered a community of around 470 people interested in various domains including ontology engineering or data publication among them. the LOV Google+ community\footnote{\url{https://plus.google.com/communities/108509791366293651606}} is now an important place to discuss, report and announce general facts related to vocabularies on the Web. The LOV dataset itself referenced 389 resources of type persons and 111 of type organization participating in vocabulary design and/or publication.
...
\begin{itemize}
\item term type (class, property, datatype or instance of the same vocabulary) which is provided for indexing by the search engine so a user can filter a search based on this information.
...
\maria{the section titles mentions before the user interface and in the text it is placed in the last paragraph, as all the section was about API I thought it was out of place, which is not, but the structure is not homogeneous with the title. If you do not understand my comment you can ignore it, it might be only in my mind. I mean that if we talk fist about apis and then about UI the title could be ``LOV Application Program Interfaces and User Interfaces''. extra minor comment. }
