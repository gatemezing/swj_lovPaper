...
\begin{itemize}
	...
	\item{cumul evol Vocabs creation date/last modif date | first last => SPARQL}
	\item{vocabs expressivity => mongo}

	\item{Median nb version per vocab | range => ES}
	\item{nb elems (classes / properties / instances) | range => ES}
	\item{Median classes / properties / instances => ES}
	\item{nb agents (organisations / persons) => ES}
	
	\item{Perc vocabs per language => mongo}
	\item{Distribution nb lang /vocabs => mongo}
	\item{Perc. vocabs with agents => mongo / SPARQL}

	\item{dist nb vocab / voaf rel type | nb | median => SPARQL}
\end{itemize}
