...
\end{description}
Based on their findings, we defined a new ranking method based on \emph{Term frequency inverse document frequency} (tf-idf) detailed in equation \ref{eq:tfidf}. 
...
\begin{tabular}{ |l|l| }
  ...
  $t$ & A vocabulary term (class, property, \\
       &  instance or datatype): $t \in V \& t \in URI$ \\
  \hline
\end{tabular}
...
* \prod_{p \in V} boost(p)
\end{split}
\end{equation}

\begin{equation}
\begin{split}
pop(t,\mathbb{D},\mathbb{V}) =  ?
\end{split}
\end{equation}
...
* norm(t,V) * pop(t,\mathbb{D},\mathbb{V})\\
: \forall t\left\{\exists q_i \in Q: t \in \sigma(q_i)\right\}
\end{split}
\end{equation}

Boost is the field-length norm, combined with the index-time field-level boost
