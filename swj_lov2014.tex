...
\begin{inparaenum}
  \item collecting input from the LOV Community;
  \item tracking and analysis of the LOV vocabulary catalogue;
  \item and giving access to LOV data using various indexes and publication methods to ease data consumption.
\end{inparaenum}
...
LOV system (code and data) is published under Creative Commons 4.0 license\footnote{\url{https://creativecommons.org/licenses/by/4.0/}} (CC BY 4.0). Three options are offered for users and applications to access LOV data:
		\begin{inparaenum}[1)] 
			\item use the LOV search engine to find the most relevant vocabulary term, vocabulary, agent matching keywords
			...
			\item use LOV system Application Program Interfaces (APIs) that provides a full access to LOV data for software applications.
		\end{inparaenum}
...
The system provides data dumps of the LOV vocabulary catalogue in notation 3 format and the LOV catalogue alongside with the latest version of each vocabulary in RDF N-quads format\footnote{\url{http://lov.okfn.org/dataset/lov/sparql}}. As illustrated in figure \ref{fig:model}, the RDF model mainly reuse the Data CATaloge Vocabulary (DCAT) which allows the representation of LOV catalogue as a \emph{dcat:Catalog} composed of vocabulary entries (\emph{dcat:CatalogRecord}) capturing information like the insertion date in LOV. Each entry points to the vocabulary itself represented by a sub class of \emph{dcat:Dataset} defined in the Vocabulary of a Friend (VOAF). This artifact contains metadata extracted by LOV such as creators, first issued date, number of occurrences of the vocabulary in Linked Open datasets. Each vocabulary is then linked to its various published versions represented by the \emph{dcat:Distribution} entity on which information such as inter vocabulary links or languages can be found.
